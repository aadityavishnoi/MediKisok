...
export function DiseaseOutbreakModule() {
  ...
  return (
    <div className="space-y-6">
      {/* Alert Header Banner */}
      <div className="bg-red-500/10 border border-red-500/30 backdrop-blur-md rounded-2xl p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="font-extrabold text-xl text-white font-display">AI Disease Outbreak & Epidemic Radar</h2>
        ...
        <button
          ...
          className="px-5 py-3 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold text-sm shadow-[0_0_20px_rgba(220,38,38,0.4)] transition-all flex items-center gap-2 shrink-0"
        >
      ...
      <div className="space-y-4">
        {OUTBREAKS.map((ob) => (
          <div key={ob.id} className="bg-white/[0.04] border border-white/10 backdrop-blur-md rounded-2xl p-6 space-y-4 hover:border-red-500/30 transition-all">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-white/10 pb-4">
              <div className="flex items-center gap-3">
                <span className={`px-3 py-1 rounded-full text-xs font-mono font-extrabold ${
                  ob.severity === 'CRITICAL' ? 'bg-red-500 text-white shadow-sm' : 'bg-amber-500/20 text-amber-300 border border-amber-500/30'
                }`}>
                  {ob.severity} RISK
                </span>
                <div>
                  <h3 className="font-extrabold text-lg text-white font-display">{ob.disease}</h3>
              ...
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <div className="text-lg font-mono font-extrabold text-red-400">{ob.cases}</div>
                  <div className="text-[11px] text-slate-400">AI Confidence: {ob.aiConfidence}</div>
                </div>
                <button
                  type="button"
                  onClick={() => handleDispatchAlert(ob.id)}
                  className="px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/15 text-white font-bold text-xs flex items-center gap-1.5 transition-all"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs bg-white/5 p-4 rounded-xl border border-white/5">
              <div>
                <span className="text-slate-400 font-medium">Symptom Cluster Pattern:</span>
                <p className="font-semibold text-slate-200 mt-0.5">{ob.symptomPattern}</p>
              </div>
              <div>
                <span className="text-slate-400 font-medium">Hospital Capacity Status:</span>
                <p className="font-mono font-bold text-amber-400 mt-0.5">86% Beds Reserved · ICU Overflow Ready</p>
              </div>
              <div>
                <span className="text-slate-400 font-medium">Automated Response Action:</span>
                <p className="font-semibold text-emerald-400 mt-0.5">Rapid Diagnostic Kits Mobilized to District Stores</p>
              </div>
            </div>
          </div>
        ))}
