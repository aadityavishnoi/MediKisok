...
export async function getSessionDetail(sessionId: string): Promise<SessionDetailResponse> {
  try {
    ...
    console.warn('[doctorDashboardService] DB query failed, returning rich demo session fallback:', err);
  }

  // Fallback demo patient session for offline testing
  const matchedDemo = DEMO_SESSIONS_FALLBACK.find((s) => s.sessionId === sessionId) || DEMO_SESSIONS_FALLBACK[0];
